      ...
      fs.stat(file, function (err, stat) {
        if (err && err.code === 'ENOENT') {
          if (req.statusCode == 404) {
            ...
            // Try for ./404.html(or catch all, if spaMode)

            middleware({
              url: (handleError ? ('/' + path.join(baseDir, errPage + defaultExt)) : req.url),
              statusCode: errStatusCode // Override the response status code
            }, res, next);
          }
        }
        else if (err) {
